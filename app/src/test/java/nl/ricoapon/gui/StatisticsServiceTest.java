...
class StatisticsServiceTest {
    ...
    void computesAggregatesOverWonLostAndInProgressGames() {
        ...
        Game won1 = Game.start("run-win-1", base).finish(true, base.plusSeconds(120));
        database.gameDao().insert(won1);
        database.gameTurnDao().insert(new GameTurn(won1.id(), 1, 10, 3, 0, 0, 0, 0, 0, 0, 20));
        database.gameTurnDao().insert(new GameTurn(won1.id(), 2, 8, 5, 0, 0, 0, 0, 0, 0, 20));

        // Won game with 4 turns: each spent 2, gained 4 -> spent 8, gained 16, net money +8.
        Game won2 = Game.start("run-win-2", base.plusSeconds(1)).finish(true, base.plusSeconds(300));
        database.gameDao().insert(won2);
        for (int turn = 1; turn <= 4; turn++) {
            database.gameTurnDao().insert(new GameTurn(won2.id(), turn, 4, 2, 0, 0, 0, 0, 0, 0, 20));
        }

        // Lost game (finished, not won) with a turn — must not affect economy or turns-to-win.
        Game lost = Game.start("run-lose", base.plusSeconds(2)).finish(false, base.plusSeconds(60));
        database.gameDao().insert(lost);
        database.gameTurnDao().insert(new GameTurn(lost.id(), 1, 99, 99, 0, 0, 0, 0, 0, 0, 20));

        // In-progress game (not finished) with no turns.
        database.gameDao().insert(Game.start("run-open", base.plusSeconds(3)));
